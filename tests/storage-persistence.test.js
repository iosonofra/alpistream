...
test('storage persists user configuration across git resets', () => {
  ...
  const origConfig = fs.existsSync(configPath) ? fs.readFileSync(configPath) : null;
  const origUserConfig = fs.existsSync(userConfigPath) ? fs.readFileSync(userConfigPath) : null;
  ...
  try {
    ...
    if (origUserConfig !== null) {
      fs.writeFileSync(userConfigPath, origUserConfig);
    } else if (fs.existsSync(userConfigPath)) {
      try { fs.unlinkSync(userConfigPath); } catch (e) {}
    }
  }
});
